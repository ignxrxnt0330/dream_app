...
class DreamStatsBloc extends Bloc<DreamStatsEvent, DreamStatsState> {
  ...
  Future<void> _fetchStats(
      FetchStats event, Emitter<DreamStatsState> emit) async {
  ...
  Future<void> _bracketChanged(
      BracketChanged event, Emitter<DreamStatsState> emit) async {
    if (event.bracket != 99999) {
      ...
      final int bracket = DateTime.now().difference(date).inDays + 1;
      await _fetchStats(FetchStats(bracket: bracket), emit);
    }
  }

  Future<void> _scrollChanged(
      StatsScrollChange event, Emitter<DreamStatsState> emit) async {
    emit(state.copyWith(
      scroll: event.scroll,
    ));
  }

  Future<void> _fetchDreams(
      FetchStatsDreams event, Emitter<DreamStatsState> emit) async {
    DateTime now = DateTime.now();
    DateTime today = DateTime(now.year, now.month, now.day);
    List<Dream> dreams = await IsarDatasource().getAllDreams(
        end: today, start: today.subtract(Duration(days: state.bracket)));
    Map<String, int> map = {};
